...
useEffect(()=>{ 
  axios.get("https://api.coinstats.app/public/v1/coins?skip=0&limit=5&currency=INR").then(res=>setData(res.data.coins)).then(console.log(data))
})
  ...
  return (
    <div>
      ...
      <div className='' >
        <div className='text-2xl font-bold  text-center  text-[#4855e0] mt-8 mb-8'>SOME POPULAR CRYPTO CURRENCIES</div>
      
        
        {data.length>0 ?<div className=" grid grid-rows-5  p-2  ">
          {data
          .map(crypto=>
        
          <div className='grid grid-cols-5 place-items-center    p-2 bg-white opacity-90'>
                    <div><p className='font-bold text-md text-center'> {crypto.rank}</p></div>

           <div><img class=" w-18 h-12 " src={crypto.icon} alt={crypto.name} /></div>
          <div><p className='font-bold text-md text-center text-'>{crypto.name}</p></div>
          <div><p className='font-bold text-md text-center'>  {crypto.symbol}</p></div>
...
<a href='/Price'>
    <p className='text-end mr-11 pr-6 mb-9 '><span className='font-bold text-gray-800 '>see more..</span></p></a>
      </div>
